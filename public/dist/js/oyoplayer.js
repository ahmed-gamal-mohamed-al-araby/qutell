...
function oyoPlayer() {
    ...
    player.changeSource = function (source) {
        var songsArray = audioPlayer.getSongs();
        var songsLength = songsArray.length;
        var count_ayahs = $('.listen_quran .data_right .sub_data.data_sound').length;
        //
        if(counter != 1) {
            var len =  (count_ayahs - songsLength) + counter ;
            var remove_len =  (count_ayahs - songsLength) + counter - 1;
            $('.listen_quran .data_right .sub_data.data_sound').eq(len).addClass('active');
            $('.listen_quran .data_right .sub_data.data_sound').eq(remove_len).removeClass('active');
        }

        // console.log(count_ayahs); 7
        // console.log(counter);      1
        // console.log(songsLength); 2




        // $('.listen_quran .data_right .sub_data.data_sound').eq((count_ayahs-songsLength)-2).removeClass('active');
        // $('.listen_quran .data_right .sub_data.data_sound').eq((count_ayahs-songsLength)-1).addClass('active');

        // if(counter != 1) {
        //     $('.listen_quran .data_right .sub_data.data_sound').eq(counter-2).removeClass('active');
        //     $('.listen_quran .data_right .sub_data.data_sound').eq(counter-1).addClass('active');
        // }
        if (source) {
            currentSong = source;
